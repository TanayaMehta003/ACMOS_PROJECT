...
# Load the CSV file
file_path = 'ids_w_v_gm_id.csv'  # Replace this with the path to your CSV file
...
# Define l_pmos values i n nanometers
l_pmos_values_nm = [180, 360, 540, 720, 900, 1080, 1260]  # Corresponding l_pmos values in nanometers

# Plot the graph
plt.figure(figsize=(10, 6))
for (x_col, y_col), l_pmos_nm in zip(zip(x_columns, y_columns), l_pmos_values_nm):
    plt.plot(data[x_col], data[y_col], label=f"L_pmos = {l_pmos_nm} nm")  # No marker for smooth lines
...
plt.legend(title="L_pmos values (nm)")
plt.grid(True)
plt.tight_layout()
